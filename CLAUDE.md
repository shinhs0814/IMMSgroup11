...
Collections:
- `users/{uid}` → `{ dietaryProfile: DietaryProfile }`
...
Auth persistence: `browserLocalPersistence` on web; `getReactNativePersistence(AsyncStorage)` on native (lazy-required to avoid loading native modules on web).
...
### Image Search (`services/imageSearch.ts`)

Used by `SearchScreen` to fetch a food image after a text query returns an `AnalysisResult`. No API key required — queries Open Food Facts, then Wikipedia (EN), then Wikipedia in the native language for Korean/Japanese queries. All requests run in parallel with a 6-second timeout.
